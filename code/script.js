...
const filterCharacters = (keep) => {
  ...
  if (category === 'accessories') {
    ...
    if (keep) {
      alert(
        `Yes, the person is a ${value}! Keep all the smokers!`
      )
      charactersInPlay = charactersInPlay.filter((person) => person [category].includes(value))
    }
    else {
...
const guess = (personToConfirm) => {
  ...
  const confirming = confirm(`Do you really want to guess ${personToConfirm}?`)
  if (confirming === true) {
...
const checkMyGuess = (personToConfirm) => {
  ...
  winOrLoseText.innerHTML = `<h1>You Won You Lucky Bastard!</h1>`
}
  // 3. Show the win or lose section
    ...
    winOrLoseText.innerHTML = `<h1> Sorry Dude, You Suck. Do You Wanna Try Again?</h1>`
  }
}
