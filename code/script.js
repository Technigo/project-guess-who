...
const start = () => {
  ...
  // we put the semicolon after each "machine"
  generateBoard();
  setSecret();
  selectQuestion();
...
const guess = (personToConfirm) => {
  let playerGuess = confirm(
    `Do you want to take a chance on ${personToConfirm}?`
  );
...
const checkMyGuess = (personToConfirm) => {
  if (personToConfirm === secret.name) {
    winoOrloseText.innerHTML = `Well done brainy! ${personToConfirm} is the one! <img src=${secret.img} alt=${secret.name}>`;
  } else {
    winoOrloseText.innerHTML = `Nooo, the correct answer would be ${secret.name}.
