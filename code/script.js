...
const filterCharacters = (keep) => {
  ...
  if (category === "accessories") {
    ...
    if (keep) {
      ...
      alert(
        `No, the person does not have ${value} hair! Remove all people with ${value}`
      );
      charactersInPlay = charactersInPlay.filter(
        (person) => person[category] !== value
      );
...
const guess = (personToConfirm) => {
  playersGuess = personToConfirm;
  if (confirm(`Are you sure you want to guess on ${playersGuess}?`)) {
    checkMyGuess(playersGuess);
  }
...
const checkMyGuess = (personToCheck) => {
  if (secret.name === personToCheck) {
    document.getElementById("winOrLoseText").innerHTML =
      "You guesed it right! Congratulations!";
  } else {
    document.getElementById("winOrLoseText").innerHTML =
