...
const numberOfGuesses = document.getElementById("guessesNumber");
...
const start = () => {
  ...
  //Dispay the guess from zero again
  charactersInPlay = CHARACTERS;
  setSecret();
  generateBoard();
  winOrLose.style.display = "none"; //Hide the win/lose board
  board.style.display = "flex"; //Display the game board
  guessesNumber = 0;
  numberOfGuesses.innerHTML = `Number of Guesses:  ${guessesNumber}/5`; //Display the guess from zero again
...
const filterCharacters = (keep) => {
  ...
  if (category === "accessories") {
    ...
    if (keep) {
      ...
      Swal.fire({
        background: "#d0c3e2",
        title: `No, the person doesnot have ${value} eyes! Remove all people that have ${value} eyes `,
      });
      charactersInPlay = charactersInPlay.filter(
        (person) => person[category] !== value
      );
...
const checkMyGuess = (confirmPerson) => {
  ...
  if (confirmPerson === secret.name) {
    winOrLoseText.innerHTML = `You guessed it right in ${guessesNumber} guesses. ${confirmPerson} is the correct answer`;
    winSound.play();
  } else {
    winOrLoseText.innerHTML = `You guessed it wrong 😫 ${secret.name} is the correct answer`;
    loseSound.play();
  }
