...
const filterCharacters = (keep) => {
  ...
  if (category === 'accessories') {
    ...
    if (keep) {
      alert(`Yes, the person has a ${value}! Keep all people that are ${value}`)
      charactersInPlay = charactersInPlay.filter((person) => person[category].includes(value))
    } else {
      alert(`No, the person doesn't have ${value}! Remove all people who is a ${value}`)
      charactersInPlay = charactersInPlay.filter((person) => !person[category].includes(value))
    }
  }
...
const guess = (personToConfirm) => {
  ...
  const confirmGuess = confirm(`Do you think ${personToConfirm} is the secret person?`)
...
// 2. Set a Message to show in the win or lose section accordingly
    winOrLoseText.innerHTML = `Good job, Your guess on ${personToCheck} is the correct person.`
    
  } else {
    winOrLoseText.innerHTML = `Guess again, ${personToCheck} is not the person.`
